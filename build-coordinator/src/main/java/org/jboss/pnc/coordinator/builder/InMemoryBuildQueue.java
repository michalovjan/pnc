...
public class InMemoryBuildQueue implements BuildQueue {
    ...
    private final Set<MDCAwareElement<BuildTask>> waitingTasks = new HashSet<>();
    ...
    private Consumer<BuildTask> taskReadyCallback;

    @Inject
    public InMemoryBuildQueue(SystemConfig systemConfig) {
        this.systemConfig = systemConfig;
    }

    @SuppressWarnings("unused")
    @Deprecated
    public InMemoryBuildQueue() {
    }
    ...
    public synchronized void removeTask(BuildTask task) {
        ...
        if (waitingTasks.remove(element)) {
    ...
    public synchronized Optional<BuildTask> getTask(BuildConfigurationAudited buildConfigAudited) {
        Optional<BuildTask> ready = readyTasks.stream()
                ...
                .findAny();
        Optional<BuildTask> waiting = waitingTasks.stream()
                .map(MDCAwareElement::get)
                .filter(bt -> bt.getBuildConfigurationAudited().equals(buildConfigAudited))
                .findAny();
    ...
    public void registerTaskReadyCallback(Consumer<BuildTask> onTaskReady) {
        this.taskReadyCallback = onTaskReady;
    }
